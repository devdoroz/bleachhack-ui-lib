...
local Category = {}; Category.__index = Category; do
	...
	function Category.new(title, icon, root)
		...
		if root.UI.Modules.AbsoluteSize.X < 180 * (#root.UI.Modules:GetChildren() - 1) then
	...
	function Category:CreateModule(title)
		local module = Module.new(title, self)
	...
	for index, category in pairs(self.Categories) do
		...
		for index, module in pairs(category.Modules) do
			...
			for index, setting in pairs(module.Settings) do
				data[category.Title][module.Title].Settings[index] = setting.Value
			end
		end
	end
...
function Bleachhack:Import(data)
	...
	for categoryName, data in pairs(data) do
		for moduleName, moduleData in pairs(data) do
			...
			self.Categories[categoryName].Modules[moduleName].Update()
			for index, value in pairs(moduleData.Settings) do
				print(value, categoryName, moduleName)
				self.Categories[categoryName].Modules[moduleName].Settings[index].Value = value
